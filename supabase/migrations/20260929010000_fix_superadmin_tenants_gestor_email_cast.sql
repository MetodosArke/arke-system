-- Fix crítico: get_superadmin_tenants() estava quebrando a listagem
-- inteira de tenants no SuperAdmin. auth.users.email é
-- character varying(255), não text — devolvê-lo direto como a coluna
-- "gestor_email" (declarada text) fazia a função inteira falhar com
-- "structure of query does not match function result type / Returned
-- type character varying(255) does not match expected type text in
-- column 14" a cada chamada, derrubando a tabela de Gestão de Tenants
-- por completo (não era um filtro indevido por campo nulo — era a RPC
-- inteira retornando erro).

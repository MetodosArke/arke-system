...
const assertStaffForLead = async (userId: string, leadId: string) => {
...
export const appRouter = router({
  ...
  crm: router({
    myOrganizations: protectedProcedure.query(async ({ ctx }) => (await getOrganizationsForUser(ctx.user.id)).filter((item) => STAFF_ROLES.includes(item.membership.role))),
    leads: router({
      list: protectedProcedure.input(organizationIdInput).query(async ({ ctx, input }) => { await assertStaffOfOrganization(ctx.user.id, input.organizationId); return listLeadsForOrganization(input.organizationId); }),
      create: protectedProcedure.input(z.object({ organizationId: z.string().uuid(), nome: z.string().trim().min(2).max(160), telefone: z.string().trim().max(40).optional(), email: z.string().email().optional(), origem: z.string().trim().max(80).optional(), notas: z.string().trim().max(4000).optional() })).mutation(async ({ ctx, input }) => { await assertStaffOfOrganization(ctx.user.id, input.organizationId); return createLead({ organizationId: input.organizationId, nome: input.nome, telefone: input.telefone, email: input.email, origem: input.origem, notas: input.notas, criadoPor: ctx.user.id }); }),
      update: protectedProcedure.input(z.object({ id: z.string().uuid(), data: z.object({ nome: z.string().trim().min(2).max(160).optional(), telefone: z.string().trim().max(40).optional().nullable(), email: z.string().email().optional().nullable(), origem: z.string().trim().max(80).optional().nullable(), responsavelId: z.string().uuid().optional().nullable(), notas: z.string().trim().max(4000).optional().nullable() }) })).mutation(async ({ ctx, input }) => { await assertStaffForLead(ctx.user.id, input.id); return updateLead(input.id, input.data); }),
      delete: protectedProcedure.input(z.object({ id: z.string().uuid() })).mutation(async ({ ctx, input }) => { await assertStaffForLead(ctx.user.id, input.id); return deleteLead(input.id); }),
      moverEstagio: protectedProcedure.input(z.object({ id: z.string().uuid(), estagio: z.enum(["novo", "contato_feito", "visita_agendada"]) })).mutation(async ({ ctx, input }) => { await assertStaffForLead(ctx.user.id, input.id); return moverEstagioLead(input.id, input.estagio); }),
      marcarPerdido: protectedProcedure.input(z.object({ id: z.string().uuid(), motivo: z.string().trim().min(2).max(500) })).mutation(async ({ ctx, input }) => { await assertStaffForLead(ctx.user.id, input.id); return marcarLeadPerdido(input.id, input.motivo); }),
      converter: protectedProcedure.input(z.object({ id: z.string().uuid() })).mutation(async ({ ctx, input }) => { await assertStaffForLead(ctx.user.id, input.id); return converterLead(input.id, ctx.user.id); }),
      atividades: protectedProcedure.input(z.object({ leadId: z.string().uuid() })).query(async ({ ctx, input }) => { await assertStaffForLead(ctx.user.id, input.leadId); return listLeadAtividades(input.leadId); }),
      criarNota: protectedProcedure.input(z.object({ leadId: z.string().uuid(), descricao: z.string().trim().min(2).max(4000) })).mutation(async ({ ctx, input }) => { const lead = await assertStaffForLead(ctx.user.id, input.leadId); return createLeadNota({ leadId: input.leadId, organizationId: lead.organization_id, descricao: input.descricao, criadoPor: ctx.user.id }); }),
    }),
  }),
  gestao: router({

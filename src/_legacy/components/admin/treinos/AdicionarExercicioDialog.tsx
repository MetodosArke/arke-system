...
export function AdicionarExercicioDialog({
  ...
  const { data: exerciciosDisponiveis = [] } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase.from("exercicios").select("id, nome, grupo_muscular, equipamento, video_url, imagem_url").order("nome");
  ...
  const selectedExercicio = exerciciosDisponiveis.find((e: any) => e.id === exercicioId) as any;
  ...
  const isYoutube = (url: string) => !!url && /youtu\.?be/.test(url);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!finalExercicioId) {
      ...
      const { data: newEx, error: exError } = await supabase
        ...
        .insert({
          nome: exercicioNome,
          grupo_muscular: grupo ? [grupo] : ["Geral"],
      ...
      if (exError) {
        toast({ title: "Erro ao criar exercício", description: exError.message, variant: "destructive" });
        setLoading(false);
        return;
      }
    ...
    // Store per-series descanso if different
    if (!mesmaConfig) {
      insertData.descanso_por_serie = seriesData.map((s) => s.descanso).join(",");
    }

    const { error } = await supabase.from("treino_exercicios").insert(insertData);
    setLoading(false);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto w-[95vw] sm:w-full">
        <DialogHeader>
          <DialogTitle className="text-base sm:text-lg">Adicionar Exercício</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
            ...
            <div>
              ...
              <SearchableSelect
                ...
                onAddNew={async (name) => {
                  await supabase
                    .from("grupos_musculares")
                    .insert({ nome: name, ordem: gruposMusculares.length + 1 });
            ...
            <div>
              ...
              <SearchableSelect
                ...
                onAddNew={async (name) => {
                  await supabase
                    .from("equipamentos")
                    .insert({ nome: name, ordem: equipamentos.length + 1 });
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mb-1">
              {grupo || equipamento ? "Filtrado por grupo/equipamento selecionados. " : ""}
              Digite para buscar ou criar novo
            </p>
            <SearchableSelect
              value={exercicioNome}
              onValueChange={handleExercicioSelect}
              options={exercicioOptions}
              placeholder="Digite o nome do exercício..."
              onAddNew={async (name) => {
                setExercicioNome(name);
                setExercicioId(null);
              }}
              addNewLabel="Criar"
            />
            {selectedExercicio && (selectedExercicio.video_url || selectedExercicio.imagem_url) && (
              <div className="mt-2 inline-block rounded-md overflow-hidden border bg-muted">
                {selectedExercicio.video_url && isYoutube(selectedExercicio.video_url) ? (
                  <img
                    src={getYoutubeThumb(selectedExercicio.video_url) || ""}
                    alt={`Preview ${selectedExercicio.nome}`}
                    className="h-24 w-auto object-cover"
                  />
                ) : selectedExercicio.video_url ? (
                  <video
                    src={`${selectedExercicio.video_url}#t=0.1`}
                    preload="metadata"
                    muted
                    playsInline
                    className="h-24 w-auto object-cover"
          ...
          {/* Video & Image - shown for new exercises */}
          {isNewExercise && (
            <>
              <div>
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm">
                Mesmas repetições e descanso para todas as séries
              </span>
            </label>
            {!mesmaConfig && (
              <Button
          ...
          <Button
            type="submit"
            className="w-full"
            disabled={!exercicioNome || loading}
          >

...
import { AdicionarExercicioTemplateDialog } from "./AdicionarExercicioTemplateDialog";

interface TemplateExerciciosProps {
  templateId: string;
  divisao: string;
}

export function TemplateExercicios({ templateId, divisao }: TemplateExerciciosProps) {
  const [addDialogOpen, setAddDialogOpen] = useState(false);
  const { toast } = useToast();
  const queryClient = useQueryClient();

  const { data: exercicios = [], isLoading } = useQuery({
    queryKey: ["template-exercicios", templateId, divisao],
    queryFn: async () => {
      const { data, error } = await (supabase
        .from("treino_template_exercicios") as any)
        .select("*, exercicios:exercicio_id(nome, video_url, imagem_url, grupo_muscular, equipamento)")
    ...
    <div className="space-y-3">
      ...
      {isLoading ? (
        ...
        exercicios.map((ex: any, idx: number) => (
          <div key={ex.id} className="flex items-start gap-2 sm:gap-3 rounded-lg bg-muted/20 p-2.5 sm:p-3 border">
            <div className="flex items-center gap-1 sm:gap-2 text-muted-foreground pt-0.5 shrink-0">
              ...
              <span className="text-xs sm:text-sm font-mono w-4 text-center">{idx + 1}</span>
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                <p className="font-semibold text-xs sm:text-sm truncate">{ex.exercicios?.nome || "—"}</p>
              ...
              <div className="flex gap-1 sm:gap-2 mt-1 flex-wrap">
                <Badge variant="secondary" className="text-[10px] sm:text-xs px-1.5 sm:px-2">🏋️ {ex.series}s</Badge>
                <Badge variant="secondary" className="text-[10px] sm:text-xs px-1.5 sm:px-2">🔁 {ex.repeticoes}</Badge>
                <Badge variant="secondary" className="text-[10px] sm:text-xs px-1.5 sm:px-2">⏱️ {ex.descanso_seg}s</Badge>
              </div>
              {ex.observacoes && (
                <p className="text-[10px] sm:text-xs text-muted-foreground mt-1.5 whitespace-pre-wrap line-clamp-2">
                  💡 {ex.observacoes}
                </p>
              )}
            </div>
      ...
      <AdicionarExercicioTemplateDialog

...
export default function ProfessorAvulsoExecucaoInline({ treinoId, alunoId, onBack }: Props) {
  ...
  if (isLoading) return <div className="space-y-4"><div className="h-8 w-48 animate-pulse rounded bg-muted" /><div className="h-64 animate-pulse rounded-lg bg-muted/50" /></div>;
  if (!treino) return <div className="text-center"><p className="text-muted-foreground">Treino não encontrado.</p><Button variant="outline" className="mt-4" onClick={onBack}>Voltar</Button></div>;

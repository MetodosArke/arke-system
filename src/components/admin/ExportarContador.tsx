...
export function ExportarContador() {
  ...
  const exportar = async () => {
    ...
    try {
      ...
      // Tudo em páginas: o mês de uma academia grande passa de mil mensalidades,
      // e o contador receberia a planilha incompleta sem ninguém saber.
      ...
      const alunosMes = await porLotes(
        [...mens.map((m) => m.aluno_id), ...avul.map((a) => a.aluno_id)],
        (lote) => supabase.from("alunos").select("id, user_id").in("id", lote)
      );
      const userDoAluno = new Map(alunosMes.map((a) => [a.id, a.user_id]));
      const perfis = await perfisDosUsuarios([...folha.map((f) => f.user_id), ...alunosMes.map((a) => a.user_id)]);
      const nome = new Map([...perfis].map(([id, p]) => [id, p.full_name]));

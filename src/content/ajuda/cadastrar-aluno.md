Em [Alunos & Prescrições](/admin/alunos), clique em **Cadastrar Aluno** e preencha nome completo, e-mail, telefone e CPF. Cadastrar e importar alunos é tarefa do gestor.

![Alunos & Prescrições: o convite de primeiro acesso no topo e a lista com plano, situação e fase.](/ajuda/telas/alunos.jpg)

## Por que o CPF é obrigatório

A matrícula gera cobrança, e o meio de pagamento não emite cobrança sem CPF. Por isso ele é exigido em todo cadastro de aluno, pela ficha, pela importação ou pelo link de matrícula. O sistema confere os dígitos na hora e avisa se o número estiver errado.

## Depois de salvar

O aluno recebe um e-mail para criar a senha. Na mesma tela você pode mandar também pelo WhatsApp, com **Enviar Ativação via WhatsApp**: o ARKE monta a mensagem com o link, e você só confirma o envio no seu WhatsApp.

Para ativar muitos alunos de uma vez, use o convite único da academia, com QR Code. Veja [Convite de primeiro acesso e guia do aluno](ajuda:primeiro-acesso-aluno).

## A lista de alunos

A lista mostra, para cada aluno, o plano (Free ou Método ARKE), a situação na academia, a assinatura, a fase da jornada e desde quando é aluno. Clique no nome para abrir a ficha completa. Em **Exportar** você baixa a lista em planilha.

Na coluna **Situação** você muda o aluno entre em dia, inadimplente e pausado. Veja [Situação do aluno](ajuda:situacao-do-aluno).

## Matricular num plano

O cadastro cria o aluno; a matrícula num plano da academia é feita na ficha, no bloco **Plano da Academia**. Veja [Cobrança do aluno](ajuda:cobranca-do-aluno).

## Quando o aluno sai

Na lista, o menu de ações do aluno tem **Desativar / Anonimizar Aluno**, o protocolo da LGPD: os dados pessoais são apagados e o que é financeiro fica guardado pelo prazo legal, sem nome. Antes disso, cancele as cobranças em aberto; o sistema não deixa sair quem ainda tem cobrança ativa no meio de pagamento, justamente para ninguém seguir sendo cobrado depois de ir embora.

> O limite de alunos ativos vem do plano contratado pela academia. Se ele for atingido, o cadastro avisa; fale com a ArkeFit para mudar de plano.

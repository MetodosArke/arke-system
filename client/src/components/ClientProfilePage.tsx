...
export function ClientProfilePage({ client, onBack, onToast }: { client: Client; onBack: () => void; onToast: (toast: Toast) => void }) {
  ...
  const readLogo = async (file?: File) => {
    ...
    try {
      ...
      const { url } = await uploadLogo.mutateAsync({ contentType, dataBase64: base64 });
      set("logoUrl", url);
    } catch (error) {
      onToast({ title: "Erro ao enviar logo", detail: error instanceof Error ? error.message : "Tente novamente." });
    } finally {
  ...
  const save = () => {
    ...
    // Esta página só edita um cadastro já existente (o botão "Novo(a)" em
    // CrudManagement usa seu próprio formulário de criação) — sem id não
    // há nada para atualizar; melhor um erro claro do que fingir sucesso.
    if (!client.id) return onToast({ title: "Não foi possível salvar", detail: "Cadastro sem identificador — volte e abra o cliente de novo." });
    // type é o valor editado no formulário — precisa vencer client.role
    // (que já era truthy antes de qualquer edição), senão a mudança nunca
    // chega a ser enviada ao servidor.
    update.mutate({ id: client.id, data: { name, email, username, module: client.module || "academia", role: type || client.role || "Cliente", status: status === "Suspenso" ? "Suspenso" : "Ativo", logoUrl, profileData: { ...profileData, type, ...(password ? { password } : {}) } } }, { onSuccess: () => { onToast({ title: "Cadastro atualizado", detail: "Todas as informações foram persistidas no Supabase." }); onBack(); }, onError: (error) => onToast({ title: "Erro ao salvar cadastro", detail: error.message }) });
  };
  return <div className="min-h-full bg-[#f7f5f0]"><div className="mx-auto max-w-5xl p-5 sm:p-8"><div className="mb-6 flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.15em] text-[#b08317]">perfil único do cliente</p><h1 className="mt-2 text-3xl font-semibold tracking-[-.05em] text-[#2b271f]">{form.name || "Cadastro do cliente"}</h1><p className="mt-2 text-sm text-[#7d887f]">Esta é a página compartilhada pelos botões Abrir cadastro e Editar.</p></div><Button variant="outline" onClick={onBack} className="rounded-xl"><ArrowLeft size={15} /> Voltar</Button></div><Card className="rounded-2xl border-[#e5ece5] bg-white"><CardHeader><CardTitle>Editar todas as informações</CardTitle></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2">{[["name","Nome ou razão social"],["document","CPF ou CNPJ"],["email","E-mail"],["phone","Telefone"],["username","Usuário de acesso"],["password","Nova senha"],["type","Perfil do cliente"],["address","Endereço completo"],["neighborhood","Bairro"],["city","Cidade"],["state","Estado"],["zipCode","CEP"]].map(([key,label]) => <label key={key} className="grid gap-1.5 text-xs font-semibold text-[#665e50]">{label}<Input type={key === "password" ? "password" : key === "email" ? "email" : "text"} value={form[key] || ""} onChange={(event) => set(key, event.target.value)} className="h-10 rounded-xl font-normal" /></label>)}<label className="grid gap-1.5 text-xs font-semibold text-[#665e50]">Plano<select value={form.plan || ""} onChange={(event) => set("plan", event.target.value)} className="h-10 rounded-xl border border-[#e2dcca] bg-white px-3 text-xs font-normal"><option value="">Selecione o plano</option>{planOptions.map((plan) => <option key={plan} value={plan}>{plan}</option>)}</select></label><div className="grid gap-2 rounded-2xl border border-[#eadfbd] bg-[#fffaf0] p-4 sm:col-span-2"><p className="text-xs font-semibold text-[#665e50]">Logo do cliente</p><div className="flex flex-wrap items-center gap-4"><img src={form.logoUrl || "/arke-logo.png"} alt="Preview da logo" className="h-20 w-20 rounded-2xl border border-[#e4e0d7] bg-[#15130f] object-contain p-2" /><div className="min-w-0 flex-1"><p className="text-xs text-[#6f786f]">Importe a logo para exibi-la no painel do cliente.</p><p className="mt-1 text-[10px] text-[#9b9488]">PNG, JPG, WEBP ou SVG.</p></div><label className={`inline-flex cursor-pointer items-center gap-2 rounded-xl px-4 py-2.5 text-xs font-semibold text-white ${uploadingLogo ? "bg-[#8b8579]" : "bg-[#15130f]"}`}><ImagePlus size={15} /> {uploadingLogo ? "Enviando..." : "Importar logo"}<input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" disabled={uploadingLogo} className="hidden" onChange={(event) => readLogo(event.target.files?.[0])} /></label></div></div><label className="grid gap-1.5 text-xs font-semibold text-[#665e50]">Status<select value={form.status} onChange={(event) => set("status", event.target.value)} className="h-10 rounded-xl border border-[#e2dcca] bg-white px-3 text-xs font-normal"><option>Ativo</option><option>Suspenso</option></select></label><div className="flex items-end"><Button disabled={update.isPending} onClick={save} className="h-10 w-full rounded-xl bg-[#15130f] text-white"><Save size={15} /> {update.isPending ? "Salvando..." : "Salvar cadastro completo"}</Button></div></CardContent></Card></div></div>;
}

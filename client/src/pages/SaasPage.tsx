...
const tenants = [
  { name: "Rede Arke Demo", type: "Conta principal", plan: "scale", users: 12, students: 698, usage: 68, initials: "RA" },
...
export default function SaasPage({ tenantName, onTenantChange, onToast }: Props) {
  ...
  const choosePlan = (plan: Plan["id"]) => {
    ...
    onToast({ title: "Plano atualizado em sandbox", detail: `A assinatura ${plan} foi simulada sem cobrança real.` });
  };
  ...
  const savePolicy = () => {
    onToast({ title: "Política salva em sandbox", detail: `${selectedUnit} · ${policyModule} · ${policyManage ? "gerenciar" : policyView ? "visualizar" : "sem acesso"}. A rota protegida está pronta para persistir essa regra.` });
  ...
  const addUnit = () => {
    ...
    onToast({ title: "Unidade criada em sandbox", detail: `${nextUnit} está pronta para receber sua política de acesso.` });
  };
  ...
  const archiveUnit = () => {
    ...
    onToast({ title: "Unidade arquivada em sandbox", detail: "O acesso da unidade foi removido da seleção ativa." });
  };
  ...
  return <div className="mx-auto max-w-[1440px] p-5 sm:p-8" style={{ ["--tenant-accent" as string]: brandColor }}>
    <div className="mb-7 flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[.16em] text-[#b88918]"><span className="h-1.5 w-1.5 rounded-full bg-[#d4a017]" /> SaaS • multi-tenant</div><h2 className="text-3xl font-semibold tracking-[-.055em] text-[#231f18] sm:text-4xl">Licenças & planos</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#766f62]">Controle organizações, usuários, branding e assinaturas recorrentes em uma única camada SaaS.</p></div><div className="flex flex-wrap gap-2"><Button onClick={() => { setOnboardingStep(1); setShowOnboarding(true); }} className="h-10 rounded-xl bg-[#15130f] px-4 text-xs font-semibold text-white hover:bg-[#2b271d]"><Rocket size={15} /> Novo onboarding</Button><Button onClick={() => onToast({ title: "Tenant criado em sandbox", detail: "O fluxo real usará o endpoint protegido de criação de organização." })} variant="outline" className="h-10 rounded-xl border-[#ded8ca] bg-[#fffdf9] text-xs font-semibold text-[#6e5b2b]"><Users size={15} /> Criar tenant</Button></div></div>

...
export default function SaasPage({ tenantName, onTenantChange, onToast }: Props) {
  ...
  const licenseRows = organizations?.map((entry) => ({ name: entry.organization.name, type: entry.organization.clientId ? "Cliente licenciado" : "Organização", plan: entry.organization.plan, status: entry.organization.status, initials: entry.organization.name.slice(0, 2).toUpperCase(), organizationId: entry.organization.id })) ?? [];

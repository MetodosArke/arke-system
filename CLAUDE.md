...
A página de vendas (`src/pages/public/Landing.tsx`) mora na **raiz de arkefit.com.br**, no mesmo build do app. Mensagem principal: retenção e inadimplência, com a catraca como apoio (decisão 9). Números de mercado só com fonte citada e link (item 11, `FONTES` em `src/lib/landing.ts`), todos do Panorama Setorial Fitness Brasil, 5ª edição (2026), comparando academias com recorrência acima de 76% às com até 25%: LTV de R$ 1.521,25 contra R$ 1.093,74, evasão de 9,68% contra 12,72% e permanência de 10,36 contra 7,93 meses. Inadimplência não tem número: não há fonte com metodologia. Sem depoimentos. A fila "ao vivo" do topo diz que é exemplo ilustrativo.

**Cores do sistema (25/09/2026).** A página vive sob o tema escuro do app (classe `dark` na raiz) e usa só os tokens dele: `primary` (o dourado), `background`, `card`, `foreground`, `muted-foreground`, e `success`/`warning` na fila de exemplo. Cor nova na página entra como token, nunca como hexadecimal, senão ela volta a se afastar do produto. Os cards de números abrem pela conclusão (**+39%** de valor por aluno, **−24%** de evasão, **+2,4 meses** de permanência) e mostram a comparação em barras rotuladas; os derivados são calculados no código a partir dos números da fonte (`PANORAMA`), e a evasão segue sem período porque o Panorama não informa.

**Números atualizados e uma página com menos texto (25/09/2026).** O primeiro card usava um estudo de 2016, com os alunos de uma academia só do Rio de Janeiro. O responsável trouxe duas substituições: "mais de 50% desistem nos primeiros 90 dias", atribuída à HFA, e "só 38% passam dos 6 meses", atribuída ao Panorama. Nenhuma das duas se confirmou em fonte aberta. No material público do Panorama, "38%" é a parcela de academias com leitor facial ou biométrico. O card passou a mostrar o LTV do próprio Panorama 2026, conferido na página da Fitness Brasil, e os três números ficaram com uma fonte só. A pedido do responsável, a página também deixou de ser só texto sem copiar o padrão dos concorrentes, que é tela do produto num notebook. "Como funciona" virou **os primeiros 90 dias de uma aluna**, com o que ela vê no app ao lado do que a academia vê na fila, em cinco momentos. Cada momento é uma regra real do sistema, e a página diz que o exemplo é ilustrativo. Recursos, Método e Implantação ficaram com uma linha cada. De passagem, apareceu um defeito que estava no ar: **no celular, as barras de comparação nunca enchiam**. A barra começa com largura zero, colada na borda do cartão, e a margem lateral negativa da checagem de visibilidade a deixava sempre "fora da tela". A margem agora é só vertical.
